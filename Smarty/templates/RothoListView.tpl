...
<!-- danzi.tn@20150825 js and css for user tree
...
<form name="advSearch" method="post" action="index.php" onSubmit="totalnoofrows();return callSearch('Advanced', '{$FOLDERID}');"> {* crmv@30967 *}
		<table cellspacing=0 cellpadding=5 width=100% class="searchUIAdv1 small" align="center" border=0>
            <tr>
				<td width="10%" class="searchUIName small" align="left" nowrap><span class="moduleName">{$APP.LBL_SEARCH}</span></td> {* crmv@31245 *}
				<td width="20%" class="small" nowrap><b><input id="matchtype_all" name="matchtype" type="radio" value="all" onclick="updatefOptionsAll(this.value);">&nbsp;<label for="matchtype_all">{$APP.LBL_ADV_SEARCH_MSG_ALL}</label></b></td>
				<td width="20%" class="small" nowrap><b><input id="matchtype_any" name="matchtype" type="radio" value="any" checked onclick="updatefOptionsAll(this.value);">&nbsp;<label for="matchtype_any">{$APP.LBL_ADV_SEARCH_MSG_ANY}</label></b></td>
				<td width="50%" class="small" align="right" valign="top"><img src="{$IMAGE_PATH}close.gif" onclick="jQuery('#advSearch').hide()" style="cursor:pointer;"></td> {* crmv@31245 *}
            </tr>
        </table>
