...
function getPriceBookName($pricebookid)
{
    ...
    $log->info("in getPriceBookName ".$pricebookid);
        global $adb, $table_prefix;
        $sql = "select * from ".$table_prefix."_pricebook where pricebookid=?";
...
function getFriendlyDate($lasttime, $nowtime = null) {
	...
	$text = $difference.' '.getTranslatedString('lbl_'.$periods[$j],'ModComments').' '.getTranslatedString('LBL_AGO','ModComments');
...
function getBlocks($module,$disp_view,$mode,$col_fields='',$info_type='')
{
	...
	$log->debug("Entering getBlocks(".$module.",".$disp_view.",".$mode.",".$col_fields.",".$info_type.") method ...");
        ...
        $block_detail = Array();
        $getBlockinfo = "";
        $query="select blockid,blocklabel,show_title,display_status from ".$table_prefix."_blocks where tabid=? and $disp_view=0 and visible = 0 order by sequence";
	...
	if($disp_view == "detail_view")
	{
        ...
        {
  		//crmv@7216e
 			$sql = "SELECT ".$table_prefix."_field.* FROM ".$table_prefix."_field WHERE ".$table_prefix."_field.tabid=? AND ".$table_prefix."_field.block IN (". generateQuestionMarks($blockid_list) .") AND ".$table_prefix."_field.displaytype IN (1,2,4) and ".$table_prefix."_field.presence in (0,2) ORDER BY block,sequence";
  			$params = array($tabid, $blockid_list);
		}
  		else
  		{
  			$profileList = getCurrentUserProfileList();
 			$sql = "SELECT ".$table_prefix."_field.* FROM ".$table_prefix."_field INNER JOIN ".$table_prefix."_def_org_field ON ".$table_prefix."_def_org_field.fieldid=".$table_prefix."_field.fieldid WHERE ".$table_prefix."_field.tabid=? AND ".$table_prefix."_field.block IN (". generateQuestionMarks($blockid_list) .") AND ".$table_prefix."_field.displaytype IN (1,2,4) and ".$table_prefix."_field.presence in (0,2) AND ".$table_prefix."_def_org_field.visible=0 ";
	...
	else
	{
		if ($info_type != '')
		{
			if($is_admin==true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2]== 0 || $module == 'Users' || $module == "Emails")
  			{
 				$sql = "SELECT ".$table_prefix."_field.* FROM ".$table_prefix."_field WHERE ".$table_prefix."_field.tabid=? AND ".$table_prefix."_field.block IN (". generateQuestionMarks($blockid_list) .") AND $display_type_check AND info_type = ? and ".$table_prefix."_field.presence in (0,2) ORDER BY block,sequence";
		...
		else
		{
			if($is_admin==true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0 || $module == 'Users' || $module == "Emails")
  			{
 				$sql = "SELECT ".$table_prefix."_field.* FROM ".$table_prefix."_field WHERE ".$table_prefix."_field.tabid=? AND ".$table_prefix."_field.block IN (". generateQuestionMarks($blockid_list).") AND $display_type_check  and ".$table_prefix."_field.presence in (0,2) ORDER BY block,sequence";
				$params = array($tabid, $blockid_list);
			}
  			else
  			{
	...
	//mycrmv@37359 projects block in helpdesk disabled
	/*
	if(isModuleInstalled('Projects') && $module == 'HelpDesk' && $disp_view != 'detail_view'){
	...
	*/
    ...
    // danzi.tn@20141024 nel caso di blocco LBL_NONCONFORMITY_EVALUATION, rimuoverlo
    
    if($module == 'Nonconformities' && $disp_view != 'detail_view'){
        $eval_label = $mod_strings['LBL_NONCONFORMITY_EVALUATION'];
        $evalBlockInfo = $getBlockInfo[$eval_label];
        unset($getBlockInfo[$eval_label]);
	}
...
function getHeaderArray()
{
    ...
    foreach($parent_tab_info_array as $parid=>$parval)
    {
        ...
        if(!$_SESSION['MorphsuitZombie'] && $parenttab == 'Settings' && $is_admin)	//crmv@2281m
        {
        	$subtabs[] = 'Settings';
        }
...
function UserCount()
{
    ...
    global $adb, $table_prefix;
    $result=$adb->pquery("select * from ".$table_prefix."_users where deleted =0", array());
    $user_count=$adb->num_rows($result);
    $result=$adb->pquery("select * from ".$table_prefix."_users where deleted =0 AND is_admin != 'on'", array());
...
function create_parenttab_data_file()
{
    ...
    global $adb, $table_prefix;
    $sql = "select parenttabid,parenttab_label from ".$table_prefix."_parenttab where visible=0 order by sequence";
    $result = $adb->pquery($sql, array());
    $num_rows=$adb->num_rows($result);
    $result_array=Array();
    for($i=0;$i<$num_rows;$i++)
...
function getQuickCreateModules($qc_module='')	//crmv@20702
{
	global $log;
	$log->debug("Entering getQuickCreateModules() method ...");
	global $adb, $table_prefix;
	global $mod_strings, $currentModule; // crmv@30967

	// vtlib customization: Ignore disabled modules.
	//crmv@20702	//crmv@31197
	if ($qc_module != '' || !Vtiger_Utils::CheckTable($table_prefix.'_quickcreate')) {
		$qc_query = "select distinct ".$table_prefix."_tab.tablabel,".$table_prefix."_tab.name from ".$table_prefix."_field inner join ".$table_prefix."_tab on ".$table_prefix."_tab.tabid = ".$table_prefix."_field.tabid where quickcreate=0 and ".$table_prefix."_tab.presence != 1";
		if ($qc_module) {
			$qc_query .= " and ".$table_prefix."_tab.name = '$qc_module'";
		}
	} else {
	...
	for($i = 0; $i < $noofrows; $i++)
	{
		...
		$tabname = $adb->query_result($result,$i,'name');
	 	...
	 	//crmv@31197
	 	$img = $adb->query_result($result,$i,'img');
	 	if ($img == '') {
	 	...
	 	if(isPermitted($tabname,'EditView','') == 'yes')
	 	{
	        ...
	        $return_qcmodule[] = $img;	//crmv@31197
		}
	}
	if(sizeof($return_qcmodule >0))
	{
		$return_qcmodule = array_chunk($return_qcmodule,3);	//crmv@31197
	}
...
function QuickCreate($module, $col_fields=array())	//crmv@26631
{
	...
	{
		$quickcreate_query = "select * from ".$table_prefix."_field where quickcreate in (0,2) and tabid = ? and ".$table_prefix."_field.presence in (0,2) and displaytype != 2 order by quickcreatesequence";
		$params = array($tabid);
	}
	else
	{
		$profileList = getCurrentUserProfileList();
 		$quickcreate_query = "SELECT ".$table_prefix."_field.* FROM ".$table_prefix."_field INNER JOIN ".$table_prefix."_def_org_field ON ".$table_prefix."_def_org_field.fieldid=".$table_prefix."_field.fieldid  WHERE ".$table_prefix."_field.tabid=? AND ".$table_prefix."_def_org_field.visible=0 and ".$table_prefix."_field.presence in (0,2) AND quickcreate in (0,2) and displaytype != 2 ";
 		$quickcreate_query.=" AND EXISTS(SELECT * FROM ".$table_prefix."_profile2field WHERE ".$table_prefix."_profile2field.fieldid = ".$table_prefix."_field.fieldid AND ".$table_prefix."_profile2field.profileid IN (". generateQuestionMarks($profileList) .") AND ".$table_prefix."_profile2field.visible = 0) ";
 		$quickcreate_query.=" ORDER BY quickcreatesequence";
...
function Button_Check($module)
{
	...
	$permit_arr = array ('EditView' => '',
        					...
        					//crmv@8719
        					  'Merge' => '',
							  'DuplicatesHandling' => '','LBL_BYPRODUCT_BTN'=>'' );
        					//crmv@8719e
...
function getTemplateDetails($templateid)
{
        ...
        $returndata =  Array();
        $result = $adb->pquery("select * from ".$table_prefix."_emailtemplates where templateid=?", array($templateid));
...
function getMergedDescription($description,$id,$parent_type,$newsletterid='',$templateid='')	//crmv@22700
{
    global $adb,$log;
...
{
    global $adb, $table_prefix;
    $sql=" select * from ".$table_prefix."_announcement inner join ".$table_prefix."_users on ".$table_prefix."_announcement.creatorid=".$table_prefix."_users.id";
    $result=$adb->pquery($sql, array());
    for($i=0;$i<$adb->num_rows($result);$i++)
    {
...
function filter_template_fields($arr,$remove){
	if (!$remove)
		return $arr;
	$type_to_remove = Array('autogenerated', 'reference', 'owner', 'multipicklist', 'password');
	$ret_arr = Array();
...
function construct_template_fields($arr,$type,$module,$related_field=''){
	...
	switch($type){
		case 'master':
			...
			break;
		case 'related':
			$ret_arr[label] = $arr[label_reference].": (".getTranslatedString($module,$module).") ".$arr[label];
...
function getEmailTemplateFields($remove=true,$module=''){ //crmv@24644
	...
	if ($res && $adb->num_rows($res) > 0){
		while ($row = $adb->fetchByAssoc($res)){
			...
			$fields = $handler->getModuleFields();
			...
			foreach ($fields as $id=>$arr){
				...
				if ($arr[type][name] == 'reference' || $arr[type][name] == 'owner'){
					...
					foreach ($arr[type][refersTo] as $related_module){
						...
						$related_fields = $handler_related->getModuleFields();
...
{
	global $adb, $table_prefix;
	//mycrmv@temp fix picklist
	$query = "select $tablename from ".$table_prefix."_$tablename inner join ".$table_prefix."_role2picklist on ".$table_prefix."_role2picklist.picklistvalueid = ".$table_prefix."_$tablename.picklist_valueid where roleid=? and picklistid in (select picklistid from ".$table_prefix."_picklist) and (picklistid <> 84 OR picklistvalueid < 78783) order by sortid";
	//mycrmv@temp fix picklist e
...
{
	global $adb, $table_prefix;
	$quer = "select activitytype from ".$table_prefix."_activity where activityid=?";
...
function getOwnerNameList($idList) {
	...
	$sql = "select user_name,id from ".$table_prefix."_users where id in (".generateQuestionMarks($idList).")";
