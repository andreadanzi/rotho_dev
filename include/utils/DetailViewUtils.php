...
function getDetailViewOutputHtml($uitype, $fieldname, $fieldlabel, $col_fields,$generatedtype,$tabid='',$module='')
{
	...
	elseif($uitype == 1099)
	{
		...
		if($value != '')
		{
			$value_label = Picklistmulti::getTranslatedPicklist($value,$fieldname);
		}
		$label_fld[] = $value_label;
	...
	elseif($uitype == 19)
	{
		if($fieldname == 'notecontent' || ($fieldname == 'description' && $module == 'Timecards')){ //crmv@22860
	...
	elseif($uitype == 52 || $uitype == 77 || $uitype == 1077  || $uitype == 101)
	{
		$label_fld[] = getTranslatedString($fieldlabel, $module);
		...
		if($uitype == 1077  ) // danzi.tn@20140220 aggiunto uitype 1077 con default vuoto
		{
			...
			{
				$users_combo = get_select_options_array(get_user_array(TRUE, "Active", $user_id), $assigned_user_id);
			}
		} else {
			if($is_admin==false && $profileGlobalPermission[2] == 1 && ($defaultOrgSharingPermission[getTabid($module)] == 3 or $defaultOrgSharingPermission[getTabid($module)] == 0))
			{
	...
	elseif($uitype == 55 || $uitype == 255)
		...
		if($tabid == 4)
           {
                   $query="select ".$table_prefix."_contactdetails.imagename from ".$table_prefix."_contactdetails where contactid=?";
	...
	elseif($uitype == 62)
	{
		...
		if($value != '')
		{
			...
			elseif($parent_module == "Projects")
			{
				...
				$projectname= $adb->query_result($result,0,"project_name");
	...
	elseif($uitype == 105)//Added for user image
	{
		...
		if($image_name != '') {
			...
			$pro_image_width = 'width="100%"';
			$pro_image_height = '';
			$label_fld[] ='<a href="'.$imgpath.'" target="_blank"><img src="'.$imgpath.'" '.$pro_image_width.' '.$pro_image_height.' alt="'.$user_name.'" title="'.$user_name.'" border="0"></a>';
			//crmv@29079e
		} else {
			$label_fld[] = '';
		}
		...
		if($value != '')
		{
			...
			if (in_array($parent_module,$relatedto)) {
				...
				foreach($entitynamefields as $entitynamefield) {
					$focus->retrieve_entity_info($value,$parent_module,false); //crmv@33983
				...
				$label_fld[] = '<a href="index.php?module='.$parent_module.'&action=DetailView&record='.$value.'">'.$parent_name.'</a>';
			} else {
				$label_fld[] = getTranslatedString($parent_module,$module);
				$label_fld[] = $value;
			}
		} else {
			$label_fld[] = getTranslatedString('Related To','Calendar');
			$label_fld[] = '';
		}
		//crmv@23515e
	}
	...
	elseif($uitype == 199){
		...
		$label_fld[] = $col_fields[$fieldname];
		$i = 0;$tmp = '';
		while ($i<strlen($col_fields[$fieldname])) {$tmp .= '*';$i++;}
		$label_fld['options'] = $tmp;
	}
	//crmv@16265e
	//crmv@18338
	elseif($uitype == 1020){
...
{
	//mycrmv@vte
	if(function_exists('getDetailAssociatedProducts_override')){
		return getDetailAssociatedProducts_override($module,$focus);
	}
	//mycrmv@vte -e
	global $log;
		...
		$output .= '
			   <tr valign="top">
				<td class="crmTableRow small lineOnTop">
					...
					<br><font color="black">'.$productname.'</font>&nbsp;&nbsp;'.$sc_image_tag.'
...
function getRelatedLists($module,$focus,$turbolift=false)	//crmv@26896
{
	...
	if ($turbolift) {
		$condition1 = '';
		if ($cur_tab_id == 9) {
			$condition1 = "and ".$table_prefix."_relatedlists.related_tabid not in (0,4)";
		}
		$sql1 = "select ".$table_prefix."_relatedlists.*
				from ".$table_prefix."_relatedlists
				left join vte_turbolift_count on (vte_turbolift_count.relation_id = ".$table_prefix."_relatedlists.relation_id and vte_turbolift_count.userid = $current_user->id)
				where ".$table_prefix."_relatedlists.tabid=? $condition1 and related_tabid not in (SELECT tabid FROM ".$table_prefix."_tab WHERE presence = 1) order by coalesce(tb_count,0) desc, sequence";
	} else {
	//crmv@26896e
		$sql1 = "select * from ".$table_prefix."_relatedlists where tabid=? and related_tabid not in (SELECT tabid FROM ".$table_prefix."_tab WHERE presence = 1) order by sequence";
	}	//crmv@26896
			...
			if($profileTabsPermission[$rel_tab_id] == 0) {
		        	if($profileActionPermission[$rel_tab_id][3] == 0) {
						...
						$focus_list[$label] = array('related_tabid' => $rel_tab_id,'relationId' =>
							$relationId, 'actions' => $actions, 'name'=>$function_name); // crmv@33097
						// END
        			}
			}
		} else {
...
//vtc
function getDetailAssociatedLinee($module,$focus)
{
	global $current_user;
	require('user_privileges/user_privileges_'.$current_user->id.'.php');
	require('user_privileges/sharing_privileges_'.$current_user->id.'.php');
	...
	<table width="98%"  border="0" align="center" cellpadding="5" cellspacing="0" class="crmTable" id="proTab">
	   <tr valign="top">
	   	<td colspan="'.$colspan.'" class="dvInnerHeader"><b>'.$app_strings['LBL_LINE_DETAILS'].'</b></td>
	   </tr>
	   <tr valign="top">
		<td width=50% class="lvtCol"><font color="red">*</font>
			<b>'.$app_strings['LBL_LINE_NAME'].'</b>
		</td>';

	$output .= '
		<td width=50% class="lvtCol" align="right"><b>'.$app_strings['LBL_AMOUNT'].'</b></td>
	   </tr>
	   	';
	...
	for($i=1;$i<=$num_rows;$i++)
	{
		...
		$output .= '
			<td class="crmTableRow small lineOnTop" align="right">
				<table width="100%" border="0" cellpadding="5" cellspacing="0">
				   <tr>
				   	<td align="right">'.convertFromDollar($amount,$rate_symbol['rate']).'</td>
				   </tr>';
		$output .= '
				</table>
			</td>';
	...
	$total = $focus->column_fields['amount'];
	$total = convertFromDollar($total,$rate_symbol['rate']);

	//Display the total, adjustment, S&H details
	$output .= '<table width="98%" border="0" align="center" cellspacing="0" cellpadding="5" class="crmTable">';
	$output .= '<tr>';
	$output .= '<td width="88%" class="crmTableRow small" align="right"><b>'.$app_strings['LBL_AMOUNT_TOTAL'].'</td>';
	$output .= '<td width="12%" class="crmTableRow small" align="right"><b>'.$total.'</b></td>';
	$output .= '</tr>';
	$output .= '</table>';
...
function getRelatedListInfoById($relationId){
	...
	$sql1 = "select * from ".$table_prefix."_relatedlists where relation_id=?";
