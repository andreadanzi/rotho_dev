...
require_once('include/utils/HTMLControlItem.php'); //ds@8s project tool
...
function get_user_array($add_blank=true, $status="Active", $assigned_user="",$private="",$read_write="")	//crmv@28496
{
	...
	if($user_array == null)
	{
		...
		{
			$temp_result[$row['id']] = getUserName($row['id'],$showfullusername);
		}
...
function get_assigned_user_or_group_name($id,$module)
{
  ...
  if($module == 'Leads')
  {
   ...
   $sql="select (case when (user_name is null) then  (".$table_prefix."_leadgrouprelation.groupname) else (user_name) end) as name from leads left join ".$table_prefix."_users on ".$table_prefix."_users.id= assigned_user_id left join ".$table_prefix."_leadgrouprelation on ".$table_prefix."_leadgrouprelation.leadid=leads.id where leads.deleted=0 and leads.id=?";

  }
  else if($module == 'Tasks')
  {
       $sql="select (case when (user_name is null) then  (taskgrouprelation.groupname) else (user_name) end) as name from tasks left join ".$table_prefix."_users on ".$table_prefix."_users.id= assigned_user_id left join taskgrouprelation on taskgrouprelation.taskid=tasks.id where tasks.deleted=0 and tasks.id=?";
  }
  else if($module == 'Calls')
  {
       $sql="select (case when (user_name is null) then  (callgrouprelation.groupname) else (user_name) end) as name from calls left join ".$table_prefix."_users on ".$table_prefix."_users.id= assigned_user_id left join callgrouprelation on callgrouprelation.callid=calls.id where calls.deleted=0 and calls.id=?";
  }
...
function getColumnFields($module)
{
	...
	if (empty($adb) || !$adb->table_exist($table_prefix.'_field')) return array();	//crmv@25671
	...
	if($cachedModuleFields === false) {
		...
		FROM ".$table_prefix."_field WHERE tabid in (" . generateQuestionMarks($tabid) . ")";
...
function getUserId($record)
{
	...
	global $adb, $table_prefix;
        $user_id=$adb->query_result($adb->pquery("select * from ".$table_prefix."_crmentity where crmid = ?", array($record)),0,'smownerid');
...
function insertProfile2field($profileid)
{
	...
	$fld_result = $adb->pquery("select * from ".$table_prefix."_field where generatedtype=1 and displaytype in (1,2,3) and tabid != 29", array());
        ...
        for($i=0; $i<$num_rows; $i++)
        {
                 ...
                 $adb->pquery("insert into ".$table_prefix."_profile2field values (?,?,?,?,?)", $params);
	}
    ...
    if($return_data === false) {
    	...
    	for($i=0; $i<$adb->num_rows($result); $i++) {
			$return_data[]=array(
				...
				intval($adb->query_result($result,$i,"visible")), // From vtiger_profile2field.visible
...
function getEmailsRelatedAccounts($record_id)
{
	...
	global $adb, $table_prefix;
	$query = "select ".$table_prefix."_seactivityrel.crmid from ".$table_prefix."_seactivityrel inner join ".$table_prefix."_crmentity on ".$table_prefix."_crmentity.crmid=".$table_prefix."_seactivityrel.crmid where ".$table_prefix."_crmentity.setype='Accounts' and activityid=?";
	$result = $adb->pquery($query, array($record_id));
...
function getEmailsRelatedLeads($record_id)
{
	...
	global $adb, $table_prefix;
	$query = "select ".$table_prefix."_seactivityrel.crmid from ".$table_prefix."_seactivityrel inner join ".$table_prefix."_crmentity on ".$table_prefix."_crmentity.crmid=".$table_prefix."_seactivityrel.crmid where ".$table_prefix."_crmentity.setype='Leads' and activityid=?";
	$result = $adb->pquery($query, array($record_id));
...
function getHelpDeskRelatedAccounts($record_id)
{
	...
	global $adb, $table_prefix;
        $query="select parent_id from ".$table_prefix."_troubletickets inner join ".$table_prefix."_crmentity on ".$table_prefix."_crmentity.crmid=".$table_prefix."_troubletickets.parent_id where ticketid=? and ".$table_prefix."_crmentity.setype='Accounts'";
        $result=$adb->pquery($query, array($record_id));
...
function getQuotesRelatedAccounts($record_id)
{
	...
	global $adb, $table_prefix;
        $query="select accountid from ".$table_prefix."_quotes where quoteid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getQuotesRelatedPotentials($record_id)
{
	...
	global $adb, $table_prefix;
        $query="select potentialid from ".$table_prefix."_quotes where quoteid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getSalesOrderRelatedAccounts($record_id)
{
	...
	global $adb, $table_prefix;
        $query="select accountid from ".$table_prefix."_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
        $accountid=$adb->query_result($result,0,'accountid');
...
function getSalesOrderRelatedPotentials($record_id)
{
	...
	global $adb, $table_prefix;
        $query="select potentialid from ".$table_prefix."_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
        $potid=$adb->query_result($result,0,'potentialid');
...
function getSalesOrderRelatedQuotes($record_id)
{
	...
	global $adb, $table_prefix;
        $query="select quoteid from ".$table_prefix."_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getInvoiceRelatedAccounts($record_id)
{
	...
	global $adb, $table_prefix;
        $query="select accountid from ".$table_prefix."_invoice where invoiceid=?";
        $result=$adb->pquery($query, array($record_id));
        $accountid=$adb->query_result($result,0,'accountid');
...
function getInvoiceRelatedSalesOrder($record_id)
{
	...
	global $adb, $table_prefix;
        $query="select salesorderid from ".$table_prefix."_invoice where invoiceid=?";
        $result=$adb->pquery($query, array($record_id));
...
//crmv@7221
/** Function to get Conact related Accouns
  * @param $record_id -- record id :: Type integer
  * @returns $soid -- soid:: Type integer
  */

function getContactsRelatedAccounts($record_id)
{
	global $log;
	$log->debug("Entering getContactsRelatedAccounts(".$record_id.") method ...");
	global $adb;
//        $query="select salesorderid from vtiger_invoice where invoiceid=?";
//        $result=$adb->pquery($query, array($record_id));
//        $soid=$adb->query_result($result,0,'salesorderid');
	$log->debug("Exiting getContactsRelatedAccounts method ...");
        return $soid;
}
//crmv@7221e
...
function useInternalMailer() {
	...
	return $adb->query_result($adb->pquery("select int_mailer from ".$table_prefix."_mail_accounts where user_id=?", array($current_user->id)),0,"int_mailer");
}
...
function is_uitype($uitype, $reqtype) {
	$ui_type_arr = array(
		...
		'_picklist_' => array(15, 16, 52, 53, 54, 55, 59, 62, 63, 66, 68, 76, 77,1077, 78, 80, 98, 101, 115, 357),
...
function getConvertDateFormat()
{
    ...
    if (strchr($current_user->date_format,"-")) $parse = "-";
    if (strchr($current_user->date_format,".")) $parse = ".";
    if (strchr($current_user->date_format,"/")) $parse = "/";

    if ($current_user->date_format == "dd".$parse."mm".$parse."yyyy" )
    {
	    $script_date = "parsedate = start_date.split('".$parse."'); ";
	    $script_date .= "start_date_year = parsedate[2]; ";
	    $script_date .= "start_date_month = parsedate[1]; ";
	    $script_date .= "start_date_day = parsedate[0]; ";

	    $script_date .= "parsedate = end_date.split('".$parse."'); ";
	    $script_date .= "end_date_year = parsedate[2]; ";
	    $script_date .= "end_date_month = parsedate[1]; ";
	    $script_date .= "end_date_day = parsedate[0]; ";
    }
    elseif ($current_user->date_format == "mm".$parse."dd".$parse."yyyy" )
    {
	    $script_date = "parsedate = start_date.split('".$parse."'); ";
	    $script_date .= "start_date_year = parsedate[2]; ";
	    $script_date .= "start_date_month = parsedate[0]; ";
	    $script_date .= "start_date_day = parsedate[1]; ";

	    $script_date .= "parsedate = end_date.split('".$parse."'); ";
	    $script_date .= "end_date_year = parsedate[2]; ";
	    $script_date .= "end_date_month = parsedate[0]; ";
	    $script_date .= "end_date_day = parsedate[1]; ";
    }
    elseif ($current_user->date_format == "yyyy".$parse."mm".$parse."dd" )
    {
	    $script_date = "parsedate = start_date.split('".$parse."'); ";
	    $script_date .= "start_date_year = parsedate[0]; ";
	    $script_date .= "start_date_month = parsedate[1]; ";
	    $script_date .= "start_date_day = parsedate[2]; ";

	    $script_date .= "parsedate = end_date.split('".$parse."'); ";
	    $script_date .= "end_date_year = parsedate[0]; ";
	    $script_date .= "end_date_month = parsedate[1]; ";
	    $script_date .= "end_date_day = parsedate[2]; ";
    }
	return $script_date;
}

function getInfoDiv($div,$content)
{
	global $app_strings;
	$info_div = '<div id="'.$div.'" style="width:340px; position:absolute; display:none; left:100px; top:100px; z-index:100000;" class="layerPopup" onMouseOut="fninvsh(\'wlastcontact\')" onMouseOut="fninvsh(\''.$div.'\')">

	      	<table class="mailClientBg" align="center" border="0" cellpadding="5" cellspacing="0" width="100%" >
	      	<tr onMouseOut="fninvsh(\'wlastcontact\')">
	      		<td style="text-align:left;" id="HandleContact" ><b>'.$app_strings["LBL_LAST_CONTACT"].'</b></td>
	      	  <td align="right">
	      			<a href="javascript:;">
	      				<img src="themes/bluelagoon/images/close.gif" border="0"  onClick="fninvsh(\'wlastcontact\')" hspace="5" align="absmiddle">
	      			</a>
	      		</td>
	        </tr>
	      	</table>
	      	...
	      	<tr>
	        	<td colspan="2">
	            <div style="border-width:3px; border-color:\'#000000\'; padding:20px;">
	            '.$content.'
	            </div>
	            </td>
	          </tr>
...
//ds@3 count of data for pulldown
function get_selection_options($noofrows,$mode=''){
	global $app_strings,$list_max_entries_per_page;
	if(isset($_REQUEST['counts']))
	{
	    $list_max_entries_per_page_temp = $_REQUEST['counts'];
	    $_SESSION['counts'] = $list_max_entries_per_page_temp;
	}
	elseif($_SESSION['counts'] != "20" && $_SESSION['counts'] != "")
	  $list_max_entries_per_page_temp = $_SESSION['counts'];
	else
	...
	if($list_max_entries_per_page_temp == $list_max_entries_per_page_backup)
	  $slected4 = " selected";
	elseif($list_max_entries_per_page_temp == "50")
	  $slected5 = " selected";
	elseif($list_max_entries_per_page_temp == "100")
	  $slected6 = " selected";
	else
	  $slected4 = " selected";
...
function getRelationTables($module,$secmodule){
	...
	if(is_array($reltables) && !empty($reltables)){
		...
		//crmv@18829
		if ($module == 'Documents' || $secmodule == 'Documents')
			$rel_array = array($table_prefix."_senotesrel"=>array("crmid","notesid"),"".$primary_obj->table_name."" => "".$primary_obj->table_index."");
		else
		//crmv@18829e
			$rel_array = array($table_prefix."_crmentityrel"=>array("crmid","relcrmid"),"".$primary_obj->table_name."" => "".$primary_obj->table_index."");
	}
	return $rel_array;
}
...
function DeleteEntity($module,$return_module,$focus,$record,$return_id) {
	...
	if ($return_module == 'Documents' && !empty($return_id)) {
...
function getModuleSequenceField($module) {
	...
	if (!empty($module)) {
		...
		if($cachedModuleFields === false) {
			...
			$seqColRes = $adb->pquery("SELECT fieldname, fieldlabel, columnname FROM ".$table_prefix."_field WHERE uitype=? AND tabid=? and ".$table_prefix."_field.presence in (0,2)", array('3', getTabid($module)));
			$sql="SELECT fieldname, fieldlabel, columnname FROM ".$table_prefix."_field WHERE uitype=? AND tabid=? and ".$table_prefix."_field.presence in (0,2)";
 ...
 */
//crmv@18038
function get_use_asterisk($id,$mode='call'){
	global $adb,$table_prefix;
...
function addToCallHistory($userExtension, $callfrom, $callto, $status, $adb, $useCallerInfo){
	...
	$sql = "insert into ".$table_prefix."_crmentity (crmid, smcreatorid, smownerid, modifiedby, setype, description, createdtime, modifiedtime, viewedtime, status, version, presence, deleted) values (".generateQuestionMarks($params).")";
...
function adjustTimezone($value, $tzoffset = 0) {
	$value = trim($value);
	if (!empty($tzoffset) && !empty($value)) {
		// controlla se c'è un'ora nella data
		if (preg_match('/[0-5]?[0-9]:[0-5]?[0-9]/', $value)) {

			//if (!isset($found[1]) || $found[1] == '') $value .= ':00';
