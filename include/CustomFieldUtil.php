...
function getCalendarCustomFields($tabid,$mode='edit',$col_fields='') {
	...
	if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
		$custquery = "select * from ".$table_prefix."_field where block=? AND ".$table_prefix."_field.tabid=? ORDER BY fieldid";		
	} else {
	    ...
	    $custquery.=" AND ".$table_prefix."_profile2field.visible = 0) "; 
 		$custquery.=" ORDER BY ".$table_prefix."_field.fieldid";
	...
	for($i=0; $i<$noofrows; $i++)
	{
		...
		if ($mode == 'edit')
			$custfld = getOutputHtml($uitype, $fieldname, $fieldlabel, $maxlength, $col_fields,$generatedtype,'Calendar',$mode, $readonly, $typeofdata);	//crmv@27597
		if ($mode == 'detail_view')
