...
function vtws_getModuleNameList(){
	...
	$sql = "select name from ".$table_prefix."_tab where isentitytype=1 and name not in ('Rss','Webmails',".
...
function vtws_getRelatedNotesAttachments($id,$relatedId) {
	...
	$sql="insert into ".$table_prefix."_senotesrel(crmid,notesid) values (?,?)";
	for($i=0; $i<$rowCount;++$i ) {
...
function vtws_saveLeadRelatedCampaigns($leadId, $relatedId, $seType) {
	...
	$result = $adb->pquery("select * from ".$table_prefix."_campaignleadrel where leadid=?", array($leadId));
	if($result === false){
		return false;
	}
	$rowCount = $adb->num_rows($result);
	for($i = 0; $i < $rowCount; ++$i) {
		...
		if($seType == 'Accounts') {
			$resultNew = $adb->pquery("insert into ".$table_prefix."_campaignaccountrel (campaignid, accountid) values(?,?)",
				array($campaignId, $relatedId));
		} elseif ($seType == 'Contacts') {
			$resultNew = $adb->pquery("insert into ".$table_prefix."_campaigncontrel (campaignid, contactid) values(?,?)",
				array($campaignId, $relatedId));
		}
		if($resultNew === false){
			return false;
...
function vtws_transferOwnership($ownerId, $newOwnerId, $delete=true) {
	...
	$sql = "update ".$table_prefix."_crmentity set smcreatorid=? where smcreatorid=?";
	$db->pquery($sql, array($newOwnerId, $ownerId));

	$sql = "update ".$table_prefix."_crmentity set smownerid=? where smownerid=?";
	$db->pquery($sql, array($newOwnerId, $ownerId));

	$sql = "update ".$table_prefix."_crmentity set modifiedby=? where modifiedby=?";
	$db->pquery($sql, array($newOwnerId, $ownerId));

	//deleting from vtiger_tracker
	if ($delete) {
		$sql = "delete from ".$table_prefix."_tracker where user_id=?";
		$db->pquery($sql, array($ownerId));
	}

	//updating created by in vtiger_lar
	$sql = "update ".$table_prefix."_lar set createdby=? where createdby=?";
	$db->pquery($sql, array($newOwnerId, $ownerId));

	//updating the vtiger_import_maps
	$sql ="update ".$table_prefix."_import_maps set assigned_user_id=? where assigned_user_id=?";
	$db->pquery($sql, array($newOwnerId, $ownerId));

	//update assigned_user_id in vtiger_files
	$sql ="update ".$table_prefix."_files set assigned_user_id=? where assigned_user_id=?";
	$db->pquery($sql, array($newOwnerId, $ownerId));

	if(Vtiger_Utils::CheckTable($table_prefix.'_customerportal_prefs')) {
	...
	if ($delete) {
		$sql = "delete from ".$table_prefix."_user2role where userid=?";
		$db->pquery($sql, array($ownerId));
	}

	//delete from vtiger_users to vtiger_role vtiger_table
	if ($delete) {
		$sql = "delete from ".$table_prefix."_users2group where userid=?";
		$db->pquery($sql, array($ownerId));
	}

	$sql = "select tabid,fieldname,tablename,columnname from ".$table_prefix."_field left join ".
	$table_prefix."_fieldmodulerel on ".$table_prefix."_field.fieldid=".$table_prefix."_fieldmodulerel.fieldid where uitype ".
...
// danzi.tn@20130511
function vtws_insertWebserviceRelatedNotes($entity_id, $doc_id) {
	global $adb,$log,$table_prefix;
	$sql = "insert into ".$table_prefix."_senotesrel(crmid,notesid) values (?,?)";
	$resultNew = $adb->pquery($sql, array($entity_id, $doc_id));
	if($resultNew === false){
		return false;
	}
	return true;
