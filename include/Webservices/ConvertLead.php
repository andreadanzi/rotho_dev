...
function vtws_convertlead($entityvalues, $user) {
	...
	// danzi.tn@201308081703.0
	$accountid = 0;
	// danzi.tn@201308081703.0e
	foreach ($availableModules as $entityName) {
		if ($entityvalues['entities'][$entityName]['create']) {
			...
			try {
				...
				// danzi.tn@201308081703.1
				if ($entityvalue['name'] == 'Contacts') {
					$sql = "SELECT ".$table_prefix."_contactdetails.contactid 
							FROM ".$table_prefix."_contactdetails
							JOIN ".$table_prefix."_crmentity on ".$table_prefix."_crmentity.crmid=".$table_prefix."_contactdetails.contactid AND ".$table_prefix."_crmentity.deleted=0
							JOIN ".$table_prefix."_account on ".$table_prefix."_account.accountid = ".$table_prefix."_contactdetails.accountid AND  ".$table_prefix."_contactdetails.accountid <> 0
							JOIN ".$table_prefix."_crmentity as account_crmentity on account_crmentity.crmid=".$table_prefix."_account.accountid AND  account_crmentity.deleted=0
							WHERE ".$table_prefix."_crmentity.crmid=".$table_prefix."_contactdetails.contactid 
							AND ((".$table_prefix."_contactdetails.firstname=? AND ".$table_prefix."_contactdetails.lastname=? ) 
									OR 
								 (".$table_prefix."_contactdetails.email <> '' AND ".$table_prefix."_contactdetails.lastname=? AND ".$table_prefix."_contactdetails.email=? ))
							AND ".$table_prefix."_crmentity.deleted=0
							AND ".$table_prefix."_account.accountid = ?";
					$result = $adb->pquery($sql, array($entityvalue['firstname'],$entityvalue['lastname'],$entityvalue['lastname'],$entityvalue['email'],$accountId));
					if ($adb->num_rows($result) > 0) {
						$entityIds[$entityName] = vtws_getWebserviceEntityId('Contacts', $adb->query_result($result, 0, 'contactid'));
						$create = false;
					}
				}
				// danzi.tn@201308081703.1e
				if ($create) {
					$entityRecord = vtws_create($entityvalue['name'], $entityObjectValues, $user);
					$entityIds[$entityName] = $entityRecord['id'];
				}
				// danzi.tn@201308081703.2
				if ($entityvalue['name'] == 'Accounts') {
					$accountIdComponents = vtws_getIdComponents($entityIds['Accounts']);
					$accountId = $accountIdComponents[1];
				}
				// danzi.tn@201308081703.2e
			} catch (Exception $e) {
				return null;
			}
		}
	...
	try {
		...
		if (!empty($accountId) && !empty($contactId) && !empty($entityIds['Potentials'])) {
			...
			$sql = "insert into ".$table_prefix."_contpotentialrel values(?,?)";
