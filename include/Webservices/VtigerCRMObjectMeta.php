...
class VtigerCRMObjectMeta extends EntityMeta {
	...
	private function retrieveMetaForBlock($block){
		...
		require('user_privileges/user_privileges_'.$this->user->id.'.php');
		//crmv@33136
		if ($tabid == 9 && !preg_match('/WSMobile.*/', $_SESSION['app_unique_key'])){ // crmv@34559
			$tabid = array(9,16);
		} else {
			$tabid = array($tabid);
		}
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] ==0){
			$sql = "select * from ".$table_prefix."_field where tabid in (".generateQuestionMarks($tabid).") and block in (".generateQuestionMarks($block).") and displaytype in (1,2,3,4) and readonly != 100"; // crmv@33097
			...
			if (count($profileList) > 0) {
				$sql = "SELECT ".$table_prefix."_field.*
						...
						WHERE ".$table_prefix."_field.tabid in (".generateQuestionMarks($tabid).")
				...
				$params = array($tabid, $block, $profileList);
			} else {
				$sql = "SELECT ".$table_prefix."_field.*
						FROM ".$table_prefix."_field
						INNER JOIN ".$table_prefix."_def_org_field
