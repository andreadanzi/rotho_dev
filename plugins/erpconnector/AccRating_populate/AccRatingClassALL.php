...
// danzi.tn@20140116 - aggiunti codici paese AT DE e CH
// danzi.tn@20140129 - aggiunti codici paese FR GB IE PL e RO
...
class AccRatingClassALL {
	
	var $entity_id = 0;
    var $account_client_type = 'PROGETTISTA';
	...
	function setEntityId($id=0,$account_client_type = 'PROGETTISTA') {
		$this->entity_id = $id;
		$this->account_client_type = $account_client_type;
	}
	
	function setVars($p_log_active, $p_ratingField, $p_codiceCorsoTargetField,$p_codiceCorsoCampagnaField, $p_codiceFatturazioneCorsoField,$p_tipoAffiliazioneField, $p_map_corsi , $p_dataCorsoCampagnaField)
	{
	...
	private function _check_temp_table() {
		global $adb;
		$delete_sql= "delete from temp_acc_ratings " .( $this->entity_id > 0 ? " WHERE accountid = ".$this->entity_id ." AND account_client_type = '".$this->account_client_type."'" : " WHERE account_client_type in ('PROGETTISTA','UTILIZZATORE','RIVENDITORE')" );
	...
	private function _insert_temp_table($account_rating_table) {
		...
		$sql .= "(accountid,categoria,gruppo,valore,eventdatetime,insdatetime,account_category,account_client_type) ";
        $sql .= " SELECT vtiger_account.accountid, ?, ?, ?, ?, GETDATE(),  vtiger_account.account_line, vtiger_account.account_client_type 
                FROM vtiger_account
			...
			foreach($val as $type_key=>$type_val) {
				foreach($type_val as $type_val_key=>$type_val_date) {
					foreach($type_val_date as $type_val_date_key=>$type_val_val) {
						...
						$adb->pquery($sql,array($type_key,$type_val_key,$type_val_val,$type_val_date_key,$key));
					}
				}
			}
		}
	...
	private function _update_account_table() {
		...
		// danzi.tn@20130920 - mettere a 0 tutti gli account che non sono stati cancellati
		$sql = "UPDATE 
	...
	private function _get_target_campaign_sql() {
		...
		$sql = "SELECT DISTINCT 
			...
			CASE WHEN ".$table_prefix."_account.account_client_type = 'PROGETTISTA' THEN
			...
			AND ".$table_prefix."_account.account_client_type IN ('PROGETTISTA','UTILIZZATORE','RIVENDITORE') 
