...
class RothoBus {
	...
	function check_web_form($email, $parms, $form_type='Form Fiere',$activitytype = "Contatto - Fiera",$acc_no="") {
		...
		$entity_ids = array_merge($entity_ids , $retval[0]);
		$entity_objects = array_merge($entity_objects , $retval[1]);
		if( !empty($acc_no) ) {
			$acc_no_retval = $this->_find_account_by_account_no($acc_no);
			$entity_ids = array_merge($entity_ids , $acc_no_retval[0]);
			$entity_objects = array_merge($entity_objects , $acc_no_retval[1]);
		}
	...
	*/
	...
	private function _find_entities_by_email($input_email) {
		...
		$entity_objects = array();
        // danzi.tn@20141105 se vuoto ritorna
        if(empty($input_email))  return array($entity_ids,$entity_objects);
        // danzi.tn@20141105e
