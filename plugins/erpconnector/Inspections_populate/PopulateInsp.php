...
//		try {
        ...
        $query = "
            ...
            AND ".$table_prefix."_products.inspection_frequency IS NOT NULL AND ".$table_prefix."_products.inspection_frequency <>'lbl_nd' AND ".$table_prefix."_products.inspection_frequency <>''
        ...
        while($row=$adb->fetchByAssoc($result))
        {
            ...
            for ($i = 1; $i <= $iqty; $i++) {
                ...
                $import_result['records_created']+=1;
            }
        }
        ...
        LEFT JOIN ".$table_prefix."_inspections AS relinsp on relinsp.salesorderid = ".$table_prefix."_inspections.salesorderid and relinsp.inspectionsid <> ".$table_prefix."_inspections.inspectionsid and  relinsp.salesorderid <> 0  
        ...
        INSERT INTO ".$table_prefix."_crmentityrel
            ...
            select 
                ...
                'Inspections'
            FROM ".$table_prefix."_inspections
            JOIN ".$table_prefix."_crmentity on ".$table_prefix."_crmentity.crmid=".$table_prefix."_inspections.inspectionsid and ".$table_prefix."_crmentity.deleted=0
            JOIN ".$table_prefix."_inspections AS relinsp on relinsp.inspection_uid = ".$table_prefix."_inspections.inspection_uid and relinsp.inspectionsid <> ".$table_prefix."_inspections.inspectionsid 
            WHERE relinsp.inspectionsid is not null
            AND NOT EXISTS (
                select 
                1 
        ...
        $update_descr = "
                        ...
                        ".$table_prefix."_inspections
                        SET 
                        ".$table_prefix."_inspections.product_description = crmprod.description
                        from 
                        ".$table_prefix."_inspections
                        INNER JOIN ".$table_prefix."_crmentity on ".$table_prefix."_crmentity.crmid = ".$table_prefix."_inspections.inspectionsid and ".$table_prefix."_crmentity.deleted=0
                        JOIN ".$table_prefix."_products on ".$table_prefix."_products.productid = ".$table_prefix."_inspections.productid
                        JOIN ".$table_prefix."_crmentity crmprod on crmprod.crmid = ".$table_prefix."_products.productid and crmprod.deleted=0
                        WHERE vtiger_inspections.product_description ='' OR  vtiger_inspections.product_description IS NULL";
        echo $update_descr."\n\n";
        $adb->query($update_descr);
