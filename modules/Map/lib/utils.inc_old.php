...
{
        // echo "<!-- MAPQUERY BEGINSQL ".$query." ENDSQL -->";
	global $adb;
	$ret = array();
	$time1 = time();
	// echo '<p>Step 1 = 0</p>';
	...
	while($row=$adb->fetchByAssoc($result))
	{
		...
		// $coord = $gc->getGeoCodeFromCache($row['id'],strtolower($row['state']),strtolower($row['city']),$row['code'],$stripped_street,strtolower($row['country']));
		$approx = "";
...
WHEN \"Low\" THEN 1000 END as map_value , 'ND' as map_aurea from vtiger_troubletickets join vtiger_contactdetails on parent_id=contactid join vtiger_contactaddress on contactid=contactaddressid join vtiger_accountscf on vtiger_contactdetails.accountid =vtiger_accountscf.accountid WHERE  mailingzip IS NOT NULL AND mailingcity IS NOT NULL "; // Andrea Danzi aggiunto type - 24.03.2012
		...
		break;
		case "Accounts":
			$query = "select accountname as name, accountaddressid as id, bill_city as city, bill_code as code, bill_country as country, bill_state as state, bill_street as street,  cf_762 as type, annualrevenue as map_value, CASE external_code WHEN NULL THEN 'ND' WHEN '' THEN 'ND' ELSE 'OK' END AS map_aurea from vtiger_accountbillads join vtiger_account on accountaddressid= vtiger_account.accountid join vtiger_accountscf on accountaddressid=vtiger_accountscf.accountid"; // Andrea Danzi aggiunto cf_762 (dovrà essere modificato per RB in  cf_762) as type - 24.03.2012
			if($extra_ids)
			{
...
left join vtiger_productcf on vtiger_productcf.productid = vtiger_inventoryproductrel.productid";
			...
			if($ids)
			{
			 	$query .= " WHERE vtiger_accountbillads.accountaddressid in ($ids)";
				if($extra_ids)
				{
				...
				if($extra_ids)
				{
					$query .= " WHERE bill_code IS NOT NULL AND bill_city IS NOT NULL AND vtiger_productcf.cf_803 LIKE '$extra_ids%' ";
					if($mindate) {
						$query .= " AND data_ordine_ven >='$mindate' ";
					}
					if($maxdate) {
...
left join vtiger_products on vtiger_products.productid = vtiger_inventoryproductrel.productid 
left join vtiger_productcf on vtiger_productcf.productid = vtiger_inventoryproductrel.productid
WHERE bill_code IS NOT NULL AND bill_city IS NOT NULL "; // Andrea Danzi aggiunto - 11.04.2012 
			if($ids)
