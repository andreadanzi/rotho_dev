...
class Visitreport extends CRMEntity {
	 ...
	 */
     // danzi.tn@20150609 missing global $table_prefix
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT ".$table_prefix."_activity.*, ".$table_prefix."_cntactivityrel.*,
			".$table_prefix."_seactivityrel.*, ".$table_prefix."_contactdetails.lastname,
...
//crmv@8398e
	...
	function get_history($id)
	{
		...
		$query = "SELECT ".$table_prefix."_crmentity.crmid, ".$table_prefix."_activity.activityid, ".$table_prefix."_activity.subject,
			...
			".$table_prefix."_crmentity.description,case when (".$table_prefix."_users.user_name not like '') then ".$table_prefix."_users.user_name else ".$table_prefix."_groups.groupname end as user_name
			FROM ".$table_prefix."_activity
			INNER JOIN ".$table_prefix."_activitycf
				ON ".$table_prefix."_activitycf.activityid = ".$table_prefix."_activity.activityid
			INNER JOIN ".$table_prefix."_seactivityrel
...
//crmv@8398e	
		...
		return getHistory($currentModule,$query,$id);
	}
		...
		if($actions) {
			...
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module,$related_module). "' class='crmbutton small edit' " .
						" type='button' onclick=\"openPopup('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\"" . // crmv@21048
