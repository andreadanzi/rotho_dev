...
$log->debug("Entering ProductToMarketprices.php ...");
...
if(isset($forfield) && $forfield != '' && $focus->popup_type != 'detailview') {
	...
	$log->debug("ProductToMarketprices.php customquery ".$query);
	...
	$autocomplete_return_function[$entity_id] = "return_product_to_marketprices($entity_id, \"$value\", \"$forfield\", \"$link_to_category\", \"$link_to_description\", \"$link_to_category_descr\");";
	$value = "<a href='javascript:void(0);' onclick='{$autocomplete_return_function[$entity_id]}closePopup();'>$value1</a>"; //crmv@21048m
}
$log->debug("Exiting ProductToMarketprices.php ...");
?>
