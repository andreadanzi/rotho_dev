...
class Vtiger_MailScannerInfo {
	...
	function updateLastscan($folderName, $rescanFolder=false) {
		...
		$scannedOn = date('d-M-Y');	//crmv@16578
	...
	function getAsMap() {
		...
		$keys = Array('scannerid', 'scannername', 'server', 'protocol', 'username', 'password', 'ssltype', 
			'sslmethod', 'connecturl', 'searchfor', 'markas', 'isvalid', 'rules', 'succ_moveto', 'no_succ_moveto');	//crmv@2043m
		foreach($keys as $key) {
	...
	function compare($otherInstance) {
		$checkkeys = Array('server', 'scannername', 'protocol', 'username', 'password', 'ssltype', 'sslmethod', 'searchfor', 'markas', 'succ_moveto', 'no_succ_moveto');	//crmv@2043m
	...
	function update($otherInstance) {
		...
		if($this->scannerid) { // This record exists in the database
			$adb->pquery("UPDATE ".$table_prefix."_mailscanner SET scannername=?,server=?,protocol=?,username=?,password=?,ssltype=?,
				sslmethod=?,connecturl=?,searchfor=?,markas=?,isvalid=?,succ_moveto=?,no_succ_moveto=? WHERE scannerid=?", 
				Array($this->scannername,$this->server,$this->protocol, $this->username, $usepassword, $this->ssltype, 
				$this->sslmethod, $this->connecturl,$this->searchfor, $this->markas,$useisvalid, $this->succ_moveto, $this->no_succ_moveto, $this->scannerid));
		} else {
			//crmv@16212
			...
			$adb->pquery("INSERT INTO ".$table_prefix."_mailscanner(scannerid,scannername,server,protocol,username,password,ssltype,
				sslmethod,connecturl,searchfor,markas,isvalid,succ_moveto,no_succ_moveto) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)", 
				Array($this->scannerid,$this->scannername,$this->server, $this->protocol, $this->username, $usepassword, 
				$this->ssltype, $this->sslmethod, $this->connecturl, $this->searchfor, $this->markas, $useisvalid, $this->succ_moveto, $this->no_succ_moveto));
			//crmv@16212 end	
		}
		//crmv@2043me
