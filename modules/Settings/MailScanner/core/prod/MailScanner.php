...
class Vtiger_MailScanner {
	...
	function markMessageScanned($mailrecord, $crmid=false) {
		...
		$adb->pquery("INSERT INTO ".$table_prefix."_mailscanner_ids(scannerid, messageid, crmid) VALUES(?,?,?)",
	...
	function isMessageScanned($mailrecord, $lookAtFolder) {
		global $adb,$table_prefix;
		$messages = $adb->pquery("SELECT * FROM ".$table_prefix."_mailscanner_ids WHERE scannerid=? AND messageid like ?", //mycrmv@3147
	...
	function GetTicketRecord($subjectOrId, $fromemail=false) {
		...
		if($ticketid) {
			if($this->_cachedTickets[$ticketid]) {
				...
				//mycrmv@3147m
				/*
				if($fromemail && !$this->LookupContactOrAccountOrLead($fromemail, $ticket_focus->column_fields[parent_id])) {
					$ticket_focus = false;
				}
				*/
				//mycrmv@3147m e
	...
	function LookupUser($email) {
		...
		$userid = false;
		$userres = $adb->pquery("SELECT id FROM ".$table_prefix."_users WHERE email1=? or email2=?", Array($email,$email));
