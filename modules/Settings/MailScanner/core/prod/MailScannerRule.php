...
class Vtiger_MailScannerRule {
	...
	function update() {
		global $adb,$table_prefix;
		if($this->ruleid) {
			$adb->pquery("UPDATE ".$table_prefix."_mailscanner_rules SET scannerid=?,fromaddress=?,toaddress=?,subjectop=?,subject=?,bodyop=?,body=?,matchusing=?
			...
			$adb->pquery("INSERT INTO ".$table_prefix."_mailscanner_rules(ruleid,scannerid,fromaddress,toaddress,subjectop,subject,bodyop,body,matchusing,sequence) 
				VALUES(?,?,?,?,?,?,?,?,?,?)", 
