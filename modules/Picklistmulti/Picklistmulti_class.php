...
class Picklistmulti {
	...
	var $has_limit;
	var $total;
	var $uitype = Array(1015,1099,1016);
	//carica tutti i campi Picklistmultilinguaggio per tutti i moduli
	function Picklistmulti($loadval = false,$module = '',$field = '',$limits=false){
...
//		echo "<br>".$adb->convert2Sql($query,$adb->flatten_array(array($tabid,$this->uitype)));die;
	...
	function getColumnsjson(){
		...
		foreach ($this->languages as $key=>$lang){
			...
			$arr[$i]['editable'] = 'true';
			$arr[$i]['editoptions'] = 'size:"20",maxlength:"30"';
			$i++;
		}
		return $arr;
	}
	...
	function getPicklistValues(){
		...
		$sql = "SELECT code_system,code,field
				...
				WHERE field = ?
				GROUP BY code_system,code,field";
		$res = $adb->pquery($sql_count,Array($this->field_name));
		if ($res){
			$this->total = $adb->query_result($res,0,'cnt');
		}	
		if ($this->has_limit){
	...
	public function getTranslatedPicklist($id = false,$fieldname,$language=false){
		...
		//mycrmv@picklistmulti
		//print_r($_REQUEST);die;
		
		if ($_REQUEST['module'] == 'Picklistmulti') {
				$qry_cname = "SELECT fieldname, columnname FROM vtiger_field WHERE fieldname = '$fieldname' AND tabid = '".getTabid($_REQUEST['field_module'])."'";
			}elseif ($_REQUEST['ajxaction'] == 'LOADRELATEDLIST') {
				$qry_cname = "SELECT fieldname, columnname FROM vtiger_field WHERE fieldname = '$fieldname' AND tabid = '".getTabid($_REQUEST['header'])."'";
			}elseif ($_REQUEST['module'] == 'Home' || $_REQUEST['module'] == 'Emails') {
				$qry_cname = "SELECT fieldname, columnname FROM vtiger_field WHERE fieldname = '$fieldname'";
			}elseif ($_REQUEST['module'] == 'Reports') { //mycrmv@38731
				$qry_cname = "SELECT fieldname, columnname FROM vtiger_field WHERE fieldname = '$fieldname'";//mycrmv@38731
			}elseif ($_REQUEST['module'] == 'Marketprices' && $fieldname=='visitreportname') { //danzi.tn@20140724 per visitreportname che è multilingua ma anche uitype = 10
				$qry_cname = "SELECT fieldname, columnname FROM vtiger_field WHERE fieldname = '$fieldname'";//danzi.tn@20140724
			}else {
				$qry_cname = "SELECT fieldname, columnname FROM vtiger_field WHERE fieldname = '$fieldname' AND tabid = '".getTabid($_REQUEST['module'])."'";
			}
		$res_cname = $adb->query($qry_cname);
		$columnname = $adb->query_result($res_cname,0,'columnname');
		$fieldname = $adb->query_result($res_cname,0,'fieldname');
...
//		echo "<br>".$adb->convert2Sql($sql,$adb->flatten_array($params));
	...
	function is_empty(){
		if (count($this->field) == 0) return 1;
		return 0;
	}
...
//		echo $adb->convert2Sql($ssql,$adb->flatten_array($params));die;
		...
		if (!$found)
			return Array(Array(' -- NOT FOUND -- '),$operator); //cmrv@33988 //mycrmv@34758
		else{
			switch($operator) {
				case 'k':
					$newsqlOperator = "n";
