...
global $default_charset, $autocomplete_return_function;
...
if(isset($forfield) && $forfield != '' && $focus->popup_type != 'detailview') {
	...
	$capoarea_test=crmv_capoarea2($entity_id,fetchUserRole($smowner));
	if ($capoarea_test == '' && $capoarea_test == null){
		$capoarea_test=1;
	}
	$autocomplete_return_function[$entity_id] = "set_extra_info(\"$entity_id\", \"$value\", \"$forfield\", \"$smowner\", \"$capoarea_test\");";
