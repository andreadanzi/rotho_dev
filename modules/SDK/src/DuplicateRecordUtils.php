...
{
	global $current_user, $table_prefix, $adb, $log;
    $log->debug("Entering DuplicateRecordUtils getDuplicateQueryRotho(".$module.")");
	//mycrmv@2707m
	$where = '';
	if (!empty($select_fields)) {
		...
		$log->debug("Entering DuplicateRecordUtils getDuplicateQueryRotho(".$module."), table cols (1 when select_fields is not empty) are ".$table_cols);
		$tbl_cols = array();
		...
		$log->debug("Entering DuplicateRecordUtils getDuplicateQueryRotho(".$module."), inner table cols are ".$inner_table_cols);
		...
		$table_cols = implode(",",$tbl_cols);
        $log->debug("Entering DuplicateRecordUtils getDuplicateQueryRotho(".$module."), table cols (2 when select_fields is empty) are ".$table_cols);
	}
	...
	if( stristr($_REQUEST['action'],'ImportStep') || ($_REQUEST['action'] == $_REQUEST['module'].'Ajax' && $_REQUEST['current_action'] == 'ImportSteplast'))
	{
		...
		else if($module == 'Vendors')
		{
			...
			if ($modObj != null && method_exists($modObj, 'getDuplicatesQuery')) {
                ...
                $log->debug("Exiting DuplicateRecordUtils getDuplicateQueryRotho(".$module.") method 3, the query is: ".$nquery);
			}
		}
	}
	else
	{
		...
		} // danzi.tn@20141105 gestione duplicati NC
		else if($module == "Nonconformities")
		{
			$nquery = "SELECT ".$table_prefix."_nonconformities.nonconformitiesid AS recordid,
				".$table_prefix."_users_last_import.deleted,$table_cols,$inner_table_cols
				FROM ".$table_prefix."_nonconformities
				INNER JOIN ".$table_prefix."_crmentity ON ".$table_prefix."_crmentity.crmid=".$table_prefix."_nonconformities.nonconformitiesid
				LEFT JOIN ".$table_prefix."_nonconformitiescf ON ".$table_prefix."_nonconformitiescf.nonconformitiesid = ".$table_prefix."_nonconformities.nonconformitiesid
				LEFT JOIN ".$table_prefix."_users_last_import ON ".$table_prefix."_users_last_import.bean_id=".$table_prefix."_nonconformities.nonconformitiesid
				LEFT JOIN ".$table_prefix."_products ON ".$table_prefix."_products.productid=".$table_prefix."_nonconformities.productid
				INNER JOIN (SELECT $inner_table_cols FROM ".$table_prefix."_nonconformities
							INNER JOIN ".$table_prefix."_crmentity ON ".$table_prefix."_crmentity.crmid = ".$table_prefix."_nonconformities.nonconformitiesid
							LEFT JOIN ".$table_prefix."_nonconformitiescf ON ".$table_prefix."_nonconformitiescf.nonconformitiesid = ".$table_prefix."_nonconformities.nonconformitiesid
							LEFT JOIN ".$table_prefix."_products ON ".$table_prefix."_products.productid=".$table_prefix."_nonconformities.productid
				WHERE ".$table_prefix."_crmentity.deleted=0  $sec_parameter
							GROUP BY ".$inner_table_cols." HAVING COUNT(*)>1) temp
				ON ".get_on_clause($field_values,$ui_type_arr,$module) ."
                                WHERE ".$table_prefix."_crmentity.deleted=0 $sec_parameter ORDER BY $inner_table_cols,".$table_prefix."_nonconformities.nonconformitiesid ASC";
		} // danzi.tn@20141105e
		else if($module == "Potentials")
		{
			$nquery = "SELECT ".$table_prefix."_potential.potentialid AS recordid,
		...
		else if($module == "Vendors")
		{
			...
			if ($modObj != null && method_exists($modObj, 'getDuplicatesQuery')) {
				...
				//mycrmv@2707me
                $log->debug("Exiting DuplicateRecordUtils getDuplicateQueryRotho(".$module.") method 4, the query is: ".$nquery);	
			}
		}
	}
	return $nquery;
...
{
	global $adb,$current_user,$table_prefix,$log;
    $log->debug("Entering DuplicateRecordUtils getDuplicateRecordSelectedFields(".$module.") method ...");
	$customView = new CustomView($module);
	$viewid = $customView->getViewId($module);
	if (empty($viewid)) {
		return false;
    ...
    $log->debug("Exiting DuplicateRecordUtils getDuplicateRecordSelectedFields(".$module.") method, the query is: ".$fieldname_query);	
