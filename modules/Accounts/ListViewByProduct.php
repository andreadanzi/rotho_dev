...
if(isset($amountrange) && $amountrange!="" )
{
	...
	$minVal = $amountrange_splitted[0];
	$maxVal = $amountrange_splitted[1];
	if( $maxVal=="250000" ) {
		$amount_where_clause = " sum( CASE WHEN {$table_prefix}_salesorder.salesorderid IS NULL OR {$table_prefix}_inventoryproductrel.quantity IS NULL THEN 0 WHEN {$table_prefix}_crmentity_sales.deleted=1 THEN 0 ELSE {$table_prefix}_inventoryproductrel.listprice*{$table_prefix}_inventoryproductrel.quantity END) >= " .$minVal . " ";
	} else {
		$amount_where_clause = " sum( CASE WHEN {$table_prefix}_salesorder.salesorderid IS NULL OR {$table_prefix}_inventoryproductrel.quantity IS NULL THEN 0 WHEN {$table_prefix}_crmentity_sales.deleted=1 THEN 0 ELSE {$table_prefix}_inventoryproductrel.listprice*{$table_prefix}_inventoryproductrel.quantity END) BETWEEN " .$minVal . " AND ". $maxVal. " ";
	}
} else {
	$smarty->assign("amountrangevalue","0-250000");
}

$extra_clause_columns = ", sum(CASE WHEN {$table_prefix}_salesorder.salesorderid IS NULL OR {$table_prefix}_inventoryproductrel.quantity IS NULL THEN 0 WHEN {$table_prefix}_crmentity_sales.deleted=1 THEN 0 ELSE {$table_prefix}_inventoryproductrel.listprice*{$table_prefix}_inventoryproductrel.quantity END) as cf_1078";
