...
function set_return_specific(product_id, product_name) {
	...
	disableReferenceField(form.account_name);	//crmv@29190
}
...
function set_return_todo(product_id, product_name) {
	...
	var form = getReturnForm(formName);
	//crmv@29190e
	form.task_parent_name.value = product_name;
	form.task_parent_id.value = product_id;
	disableReferenceField(form.task_parent_name);	//crmv@29190
}
...
function set_return_address(account_id, account_name, bill_street, ship_street, bill_city, ship_city, bill_state, ship_state, bill_code, ship_code, bill_country, ship_country,bill_pobox,ship_pobox) {
	...
	disableReferenceField(form.elements["account_name"]);	//crmv@29190
...
function set_return_contact_address(account_id, account_name, bill_street, ship_street, bill_city, ship_city, bill_state, ship_state, bill_code, ship_code, bill_country, ship_country,bill_pobox,ship_pobox ) {
	...
	//crmv@29190e
	//crmv@17789
	if(typeof(form.elements["account_name"]) != 'undefined')
		form.elements["account_name"].value = account_name;
...
//vtc
function show_hierach(accountid) {
	$("status").style.display="inline";
      var url = "module=Accounts&action=AccountsAjax&file=GetHierarchyAjax&accountid="+accountid;
      new Ajax.Request(
            'index.php',
              {queue: {position: 'end', scope: 'command'},
                      method: 'post',
                      ...
                      onComplete: function(response) {
						...
						var obj = getObj('accountHierarcContent');
						if(obj)
							obj.innerHTML = str;
						fnvshobj(this,'accountHierarc');
                      }
              }
              );		
}

function showHideHierarch(button, accountid){

	button = getObj(button);

	if (getObj('accountHierarc').style.display == 'none'){
		show_hierach(accountid,1);
		button.value = alert_arr.LBL_HIDEHIERARCH;
	}else{
		fninvsh('accountHierarc');
		button.value = alert_arr.LBL_SHOWHIERARCH;
	}

}
function HideHierarch(){
	
	button = getObj('ShowHierarch');
	fninvsh('accountHierarc');
	button.value = alert_arr.LBL_SHOWHIERARCH;
}
//vtc e