...
// force activities and activity history and documents in the exploded state
/*if (empty($_SESSION['relatedlist']['Accounts'])) {
 // get relation ids
 $res = $adb->query('SELECT relation_id,label FROM vtiger_relatedlists WHERE tabid = 6');
 $relids = array();
 if ($res) {
  while ($row = $adb->fetchByAssoc($res)) {
   $relids[$row['relation_id']] = $row['label'];
  }
 }
 // inject it into session array
 foreach ($relids as $rid=>$lab) {
  $_SESSION['relatedlist']['Accounts'][$rid] = $lab;
 }
}
*/
...
//crmv@7216
if(vtlib_isModuleActive('Fax') && isPermitted("Fax","EditView",'') == 'yes')	//crmv@26147
{
	$smarty->assign("SENDFAXBUTTON","permitted"); 

	$smarty->assign("FAX", $focus->column_fields['fax']); 
} 
//crmv@7216e
