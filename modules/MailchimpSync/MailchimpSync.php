...
class MailchimpSync extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name;
	var $table_index= 'mailchimpsyncid';
	...
	var $list_fields_name = Array(
		...
		'Mailchimp Name'=> 'mailchimp_name',
		'Assigned To' => 'assigned_user_id',
		'Campaign Type' => 'mailchimp_type',
		'Campaign State' => 'mailchimp_state',
		'Last Synchronization' => 'lastsynchronization',
		'Created Time' => 'createdtime',		
	);

	// Make the field link to detail view from list view (Fieldname)
	var $list_link_field = 'mailchimp_name';
	...
	var $popup_fields = Array('mailchimp_name');

	// Placeholder for sort fields - All the fields will be initialized for Sorting through initSortFields
	var $sortby_fields = Array();

	// For Alphabetical search
	var $def_basicsearch_col = 'mailchimp_name';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'mailchimp_name';

	// Required Information for enabling Import feature
	var $required_fields = Array('mailchimp_name'=>1);

	// Callback function list during Importing
	var $special_functions = Array('set_import_assigned_user');

	var $default_order_by = 'mailchimp_name';
	var $default_sort_order='ASC';
	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vte_field.fieldname values.
	var $mandatory_fields = Array('createdtime', 'modifiedtime', 'mailchimp_name');
	//crmv@10759
	var $search_base_field = 'mailchimp_name';
	//crmv@10759 e
	
	function __construct() {
		global $log, $currentModule, $table_prefix;
		$this->table_name = $table_prefix.'_mailchimpsync';
		$this->customFieldTable = Array($table_prefix.'_mailchimpsynccf', 'mailchimpsyncid');
		$this->entity_table = $table_prefix."_crmentity";
		$this->tab_name = Array($table_prefix.'_crmentity', $table_prefix.'_mailchimpsync', $table_prefix.'_mailchimpsynccf', $table_prefix.'_mailchimpsyncdiff');
		$this->tab_name_index = Array(
			$table_prefix.'_crmentity' => 'crmid',
			$table_prefix.'_mailchimpsync'   => 'mailchimpsyncid',
			$table_prefix.'_mailchimpsynccf' => 'mailchimpsyncid'
		);
		$this->list_fields = Array(
			/* Format: Field Label => Array(tablename, columnname) */
			// danzi.tn@20140505 modifica colonne elenco related e anche su vtiger_cvcolumnlist
			'Mailchimp Name'=> Array($table_prefix.'_mailchimpsync', 'mailchimp_name'),
			'Assigned To' => Array($table_prefix.'_crmentity','smownerid'),
			'Campaign Type' => Array($table_prefix.'_mailchimpsync','mailchimp_type'),
			'Campaign State' => Array($table_prefix.'_mailchimpsync','mailchimp_state'),
			'Last Synchronization' => Array($table_prefix.'_mailchimpsynccf','lastsynchronization'),
...
//crmv@8398e	
		...
		return getHistory('Mailchimp',$query,$id);
	}
	/** 
